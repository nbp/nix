...
/* Store a string in the Nix store as a source file that can be used as an
   input by derivations.  An uid different than -1 is used to indicate a
   secret file. */
static void toFile_impl(EvalState & state, uid_t uid, const Pos & pos, Value * * args, Value & v)
{
    PathSet context;
    string name = state.forceStringNoCtx(*args[0], pos);
    ...
    Path storePath = settings.readOnlyMode
        ...
        : store->addTextToStore(name, contents, refs, uid, state.repair);
...
{
    toFile_impl(state, /* no secret */ -1, pos, args, v);
}
...
{
    uid_t uid = getuid();
    toFile_impl(state, /* secret */ uid, pos, args, v);
}
